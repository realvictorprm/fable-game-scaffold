...
module PIXI =
    ...
    type [<AllowNullLiteral>] [<Import("EventEmitter","pixi.js")>] EventEmitter() =
    ...
    and [<AllowNullLiteral>] [<Import("DisplayObject","pixi.js")>] DisplayObject() =
    ...
    and [<AllowNullLiteral>] [<Import("Container","pixi.js")>] Container() =
    ...
    and RendererOptions =
        ...
        [<Emit("$0.clearBeforeRendering{{=$1}}")>]
        abstract ClearBeforeRendering : bool with get, set
        [<Emit("$0.preserveDrawingBuffer{{=$1}}")>]
        abstract PreserveDrawingBuffer : bool with get, set
        [<Emit("$0.forceFXAA{{=$1}}")>]
        abstract ForceFXAA : bool with get, set
        [<Emit("$0.roundPixels{{=$1}}")>]
        abstract RoundPixels : bool with get, set
        [<Emit("$0.backgroundColor{{=$1}}")>]
        abstract BackgroundColor : float with get, set

    and [<Import("SystemRenderer","pixi.js")>] SystemRenderer(system: string, ?width: float, ?height: float, ?options: RendererOptions) =
        inherit EventEmitter()
    ...
    and [<Import("CanvasRenderer","pixi.js")>] CanvasRenderer(?width: float, ?height: float, ?options: RendererOptions) =
    ...
    and [<Import("WebGLRenderer","pixi.js")>] WebGLRenderer(?width: float, ?height: float, ?options: RendererOptions) =
        inherit SystemRenderer("")
    ...
    and [<Import("Text","pixi.js")>] Text(?text: string, ?style: TextStyle, ?resolution: float) =
        inherit Sprite()
    ...
    module extras =
        ...
        and [<Import("extras.BitmapText","pixi.js")>] BitmapText(text: string, ?style: BitmapTextStyle) =
            inherit Container()
